...
/*
	...
	Tasks are processed FIFO. Multiple tasks for the same instance simply rewrite
	the same target path in order, so the on-disk file ends up reflecting the
	latest push - we never drop a snapshot because the previous write was still
	in flight (losing a round in the recovery file would defeat the point).
*/
