	...
	*/
...

	{
		...
		/*
			...
			We pre-validate the recovery arena exists on disk before letting it override
			the configured one - otherwise a since-deleted arena would propagate into
			apply()'s catch path, whose `previous_arena` fallback is empty on first_time
			and would fail the second rechoose too.
		*/
		...
		if (const auto target = crash_recovery.load(last_start.port, recovery_enabled)) {
			if (::server_choose_arena_file_by(std::string(target->arena)).empty()) {
				LOG(
					"Recovery file references arena '%x' which is not on disk. Discarding the recovery.",
					target->arena
				);
				crash_recovery.discard();
			}
			else {
